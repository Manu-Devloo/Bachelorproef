...
\title[Geïsoleerde challenge-omgevingen per speler of team]%
...
% If you included tables, uncomment the appropriate lines.
\listoftables
...
De re\"ele validatie gebruikte twee Lima-VM's op een gedeeld \texttt{user-v2}-netwerk. De eerste VM draaide CTFd en de plugin; de tweede VM draaide de Docker Engine voor challenge-containers. De belangrijkste setupstappen waren:

\begin{verbatim}
limactl network create bp-shared-net --gateway 192.168.150.1/24

limactl start --name=bp-ctfd-vm \
  --network=lima:bp-shared-net \
  --cpus=2 --memory=4 --disk=20 \
  --mount=/Users/manu/Documents/GitHub/Bachelorproef:w \
  template:docker-rootful -y

limactl start --name=bp-docker-vm \
  --network=lima:bp-shared-net \
  --cpus=2 --memory=4 --disk=20 \
  template:docker-rootful -y
\end{verbatim}

Op de runtime-VM werd de Docker-daemon voor deze wegwerpvalidatie ook op TCP-poort \texttt{2375} beschikbaar gemaakt. Dat gebeurde enkel om remote Docker-aansturing te testen; voor productie blijft TLS-beveiliging vereist.
...
Daarna werd de CTFd-stack op de control-VM gestart met de runtime-VM als Docker-control endpoint en als publieke challenge-host:
...
De smoke test werd vanuit de CTFd-VM uitgevoerd, omdat de macOS-host het interne Lima-subnet in deze netwerkmodus niet rechtstreeks kon bereiken:
...
Resultaat: op 24 maart 2026 werd bevestigd dat CTFd op een eerste Lima-VM challenge-containers kon starten, stoppen en opruimen op een tweede Lima-VM. De smoke test rapporteerde \texttt{Smoke test passed}. Na afloop bevestigden \texttt{docker ps -a} en \texttt{docker network ls} op de runtime-VM dat geen plugin-beheerde containers of netwerken waren achtergebleven. De volledige log is opgenomen in \path{POC-CTFd/docs/03_real-vm-validation.md}.
