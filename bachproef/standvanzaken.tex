...
Een Capture The Flag (CTF)-event is een cybersecuritywedstrijd waarbij deelnemers technische uitdagingen moeten oplossen om een geheime reeks tekens — de \textit{flag} — te bemachtigen en in te dienen voor punten~\autocite{Innovirtuoso2025}. CTF-events worden ingezet als educatief middel, als rekruteringstool en als competitieve sport binnen de cybersecuritygemeenschap.
...
CTFd is een open-source CTF-platform gebouwd in Python met het Flask-framework~\autocite{CTFdGitHub}. Het biedt een gebruiksvriendelijke interface voor zowel organisatoren als deelnemers en is modulair uitbreidbaar via een plugin-systeem.

\subsection{Architectuur van CTFd}

CTFd bestaat uit een Flask-backend, een relationele database (standaard SQLite, in productie MySQL of PostgreSQL) en een frontend op basis van Bootstrap. De applicatie draait typisch achter een reverse proxy zoals Nginx. Alle functionaliteit is bereikbaar via een REST API.
...
\subsection{Low-hanging fruit: basismaatregelen}
...
\begin{itemize}
  \item \textbf{Privilege management}: containers draaien standaard als root; dit moet worden voorkomen via \texttt{--user}.
  \item \textbf{Read-only filesystems}: via \texttt{--read-only} worden schrijfacties beperkt.
  \item \textbf{Capabilities droppen}: Linux capabilities die niet nodig zijn (bijv.\ \texttt{NET\_ADMIN}, \texttt{SYS\_ADMIN}) worden verwijderd via \texttt{--cap-drop ALL}.
  ...
  \item \textbf{No new privileges}: het \texttt{--security-opt no-new-privileges}-vlag voorkomt privilege-escalatie.
\end{itemize}
